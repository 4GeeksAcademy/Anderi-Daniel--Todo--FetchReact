...
const TodoList = () => {
    ...
    return (
        <div className="notebook">
            <h1>To Do List</h1>
            <input onKeyDown={handleInputEnter} onChange={e => setTask(e.target.value)} value={task} placeholder='add task here' type='text'></input>
            <ol>
                {list.map((item, index) => (<li key={index}>{item.label}<button onClick={()=> handleDeleteTask(item.label)}>X</button></li>))}
            </ol>
        </div >
    )
}
